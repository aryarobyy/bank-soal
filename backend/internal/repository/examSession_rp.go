...
func (r *examSessionRepository) GetUserSession(ctx context.Context, examId int, userId int, limit int, offset int) ([]model.ExamSession, int64, error) {
	var (
	...
	if err := r.db.WithContext(ctx).
		Where("id = ? AND user_id = ?", examId, userId).
