...
func QuestionRoutes(r *gin.RouterGroup, question *controller.QuestionController, userRepo repository.UserRepository) {
	routes := r.Group("/question")
	{
		...
		auth.Use(middleware.AuthMiddleware(userRepo))
		{
			...
			auth.GET("/creator/subject", question.GetByCreatorNSubject)
			auth.GET("/creator/diff", question.GetByCreatorNDifficult)
		}
	}
}
