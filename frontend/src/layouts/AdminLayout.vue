	...
	<div class="font-sans bg-gray-50 min-h-screen flex flex-col">
		<header
			...
			<div class="flex items-center gap-3">
				<button @click="toggleSidebar"
					class="p-2 text-gray-600 rounded-md md:hidden hover:bg-gray-100 focus:outline-none">
			...
			<nav class="items-center hidden gap-10 md:flex">
				<router-link to="/admin/dashboard"
					class="font-semibold no-underline text-gray-600 hover:text-red-600">Dashboard</router-link>
			</nav>
			...
			<div class="relative" ref="dropdownContainer">
				...
				<transition enter-active-class="transition ease-out duration-100"
					...
					<div v-if="showProfileDropdown"
						...
						<div class="py-1">
							<router-link v-if="user" :to="`/admin/profile/${user.id}`" @click="showProfileDropdown = false"
		...
		<div class="flex flex-1 relative">
			<div v-if="isSidebarOpen" @click="isSidebarOpen = false"
				class="fixed inset-0 z-40 bg-black bg-opacity-50 md:hidden glass-effect"></div>
			...
			<aside :class="[
				...
				<div class="h-full overflow-y-auto p-4 space-y-2">
					...
					<router-link v-for="(item, index) in menuItems" :key="index" :to="item.path" @click="isSidebarOpen = false"
