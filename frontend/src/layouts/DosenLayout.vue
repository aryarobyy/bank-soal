  ...
  <div class="font-sans bg-gray-50 min-h-screen flex flex-col">
    <header class="sticky top-0 z-30 flex items-center justify-between px-4 md:px-[5%] py-4 bg-white border-b border-gray-200 shadow-sm">
      <div class="flex items-center gap-3">
        <button @click="toggleSidebar" class="p-2 text-gray-600 rounded-md md:hidden hover:bg-gray-100">
          <i class="fas fa-bars text-xl"></i>
        </button>
        <router-link to="/dosen/dashboard" class="text-2xl md:text-3xl font-bold no-underline text-indigo-600">
          Latih.in
        </router-link>
      </div>

      <nav class="items-center hidden gap-10 md:flex">
        <router-link to="/dosen/dashboard" class="font-semibold text-gray-600 hover:text-indigo-600">Dashboard</router-link>
      </nav>

      <div class="relative" ref="dropdownContainer">
        <div @click="toggleProfileDropdown" class="cursor-pointer flex items-center gap-2">
           <span class="hidden md:block text-sm font-medium text-gray-700">{{ user?.name || 'Dosen' }}</span>
           <UserCircle class="w-10 h-10 text-gray-600 transition-colors hover:text-indigo-600" />
        </div>
        ...
        >
          <div v-if="showProfileDropdown" class="absolute top-12 right-0 z-50 w-64 p-2 bg-white rounded-lg shadow-xl border border-gray-100 origin-top-right">
              ...
              <div class="py-1">
                ...
                <a href="#" @click.prevent="logout" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 rounded hover:bg-indigo-50 hover:text-indigo-600">
    ...
    <div class="flex flex-1 relative">
      <div v-if="isSidebarOpen" @click="isSidebarOpen = false" class="fixed inset-0 z-40 bg-black/50 md:hidden"></div>
      ...
      >
        <div class="h-full overflow-y-auto p-4 space-y-2">
          <div class="flex justify-between items-center mb-6 md:hidden">
            ...
            <button @click="isSidebarOpen = false" class="text-gray-500 hover:text-indigo-600">
          ...
          <router-link 
            ...
            :class="isActive('/dosen/dashboard') ? 'bg-indigo-50 text-indigo-600' : 'text-gray-600 hover:bg-gray-50'">
          ...
          <router-link 
            ...
            @click="isSidebarOpen = false"
            class="flex items-center gap-3 px-4 py-3 font-medium rounded-lg transition-colors" 
            :class="isActive('/dosen/soal') ? 'bg-indigo-50 text-indigo-600' : 'text-gray-600 hover:bg-gray-50'">
